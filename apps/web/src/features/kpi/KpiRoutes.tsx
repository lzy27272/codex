...
import { CtripAuthorizationAction } from './CtripAuthorizationAction';
...
function ScorecardBody({ identity, grantedPermissions, card, reload }: {
    ...
    const submitManual = async () => {
        try {
            await command.run(() => submitManualKpiScore(identity, card.id, { ...manual, score: Number(manual.score), evaluatorAssignmentId: identity.businessActorAssignmentId }));
            await reload();
        }
        catch (error) {
    ...
    const submit = async () => {
        ...
        try {
            await command.run((key) => submitInspection(identity, {
                assignmentId: identity.businessActorAssignmentId,
